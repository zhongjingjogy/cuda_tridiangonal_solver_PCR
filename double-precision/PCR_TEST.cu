...
double GpuResultCheck(double *a, double *b, double *c, double *rhs, double *old, int Nx) {
    double v = -1.0;
    double v1 = fabs(b[0] * old[0] + c[0] * old[1] - rhs[0]);
    if (v < v1) {
        v = v1;
    }
...
int Test(size_t diagonal_size) {
    ...
    for (int count = 0; count < trynumber; count++) {
        for (int i = 0; i < diagonal_size; i++) {
            alist[i] = -1.0 + 0.1 * double(rand()) / double(RAND_MAX);
            blist[i] = 2.0 + 0.1 * double(rand()) / double(RAND_MAX);
            clist[i] = -1.0 + 0.1 * double(rand()) / double(RAND_MAX);
            dlist[i] = 1.0 + 10.0 * double(rand()) / double(RAND_MAX);
            xlist[i] = 0.0f;
        ...
        gpuresultcheck[count] =
            GpuResultCheck(ma, mb, mc, mrhs, mx, diagonal_size) < 1.0e-8;
