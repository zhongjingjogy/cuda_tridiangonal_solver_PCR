...
void TDMA(std::vector<float> &a, std::vector<float> &b, std::vector<float> &c, std::vector<float> &rhs, std::vector<float> &old, int Nx) {
    float *beta = new float[Nx];
    float *y = new float[Nx];
    beta[0] = c[0] / b[0];
    for(int nx=1; nx<Nx-1; nx++) {
...
void TDMAsolve(double* a, double* b, double* c, double* d, int n) {
    /*
...
// from: https://en.wikibooks.org/wiki/Algorithm_Implementation/Linear_Algebra/Tridiagonal_matrix_algorithm
void TDMAsolve(std::vector<float> &a, std::vector<float> &b, std::vector<float> &c, std::vector<float> &d, int n) {
    /*
    // n is the number of unknowns
