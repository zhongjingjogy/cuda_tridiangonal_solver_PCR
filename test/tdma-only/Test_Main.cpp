...
double CpuResultCheck(float *a, float *b, float *c, float *rhs, float *old,
                      int Nx) {
...
double CpuResultCheck(std::vector<float> &a, std::vector<float> &b,
                      std::vector<float> &c, std::vector<float> &rhs,
                      std::vector<float> &old, int Nx) {
    double tolerance = 1.0e-6;
...
int Test2() {
    ...
    std::vector<float> mb(x.size());
    std::vector<float> mc(x.size());
    for (int nx = 0; nx < Nx; nx++) {
        y[nx] = rhs[nx];
        ma[nx] = a[nx];
        mb[nx] = b[nx];
